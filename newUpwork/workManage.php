...
if (isset($_POST['delete_submit'])) {
    $work->deleteWork($_POST['delete_id']);
}
...
<main style="margin: 3% 10% 0">
    ...
    <div class="container">
        <?php if (isset($_GET['msg'])) : ?>
            <?php switch (($_GET['msg'])) : ?>
<?php case 'delete': ?>
                    <div id="delete_ok" class="alert alert-danger">
                        <strong class="mr-3">消息：</strong>删除成功 (可能需要手动刷新更新数据)
                    </div>
                    <?php break; ?>
                <?php case 'update': ?>
                    <div id="delete_ok" class="alert alert-info">
                        <strong class="mr-3">消息：</strong>更新成功 (可能需要手动刷新更新数据)
                    </div>
                    <?php break; ?>
                <?php endswitch; ?>
        <?php endif; ?>
            ...
            <div id="select_box" class="collapse" data-parent="#manage">
                <div class="card-body">
                    <form class="form-inline" action="workManage.php" id="select" method="post">
                        ...
                        <input class="btn btn-primary" id="select_submit" type="submit" name="select_submit" value="查询">
                    </form>
                </div>
            </div>
        </div>
            ...
            <div id="insert_box" class="collapse" data-parent="#manage">
                <div class="card-body">
                    <form class="form-inline" action="workManage.php" id="insert" method="post">
                        ...
                        <label for="insert_need_upload">需要上交</label>
                        ...
                        <input class="btn  btn-primary" id="insert_submit" type="submit" name="insert_submit"
                               value="插入">
                    </form>
                </div>
            </div>
    ...
    <div class="container mt-5">
        <?php if (isset($_GET['update_id'])) :$workInfo = getWorkInfo($_GET['update_id']); ?>
            <div class="alert alert-info">
                <strong class="mr-3">修改</strong>正在修改下面这条数据

                <form class="form-inline mt-3" action="workManage.php" id="update" method="post">
                    <div class="form-group">
                        <label for="update_name">作业名：</label>
                        <input class="form-control" id="update_name" name="update_name" type="text" required
                               value="<?php echo $workInfo[0][1] ?>">

                        <label for="update_subject">科目：</label>
                        <select class="form-control" id="update_subject" name="update_subject" required>
                            <option value="">未选择</option>
                            <?php foreach ($courseArray as $cor_item): ?>
                                <option value="<?php echo $cor_item['id'] ?>"
                        ...
                        <label for="update_start">开始日期
                            <input class="form-control" id="update_start" name="update_start" type="date"
                                   value="<?php echo $workInfo[0][3] ?>">
                        </label>

                        <label for="update_end">结束日期
                            <input class="form-control" id="update_end" name="update_end" type="date"
                                   value="<?php echo $workInfo[0][4] ?>">
                        </label>
                    </div>
                    <div class="form-group">
                        <label for="update_need_upload">需要上交
                            <select class="form-control" id="update_need_upload" name="update_need_upload" required>
                                <option value="">未选择</option>
                                <option value="0" <?php if ($workInfo[0][7] == '0') echo "selected"; ?> >否
                                </option>
                                <option value="1" <?php if ($workInfo[0][7] == '1') echo "selected"; ?> >是
                                </option>
                            </select>
                        </label>

                        <label for="update_annex">课件名
                            <input class="form-control" id="update_annex" name="update_annex" type="text"
                                   value="<?php echo $workInfo[0][5] ?>">
                        </label>

                        <label for="update_remarks">备注
                            <textarea class="form-control" id="update_remarks" name="update_remarks" rows="1"
                                      cols="30"><?php echo $workInfo[0][6] ?></textarea>
                        </label>
                    </div>
                </form>
                <div class="btn-group mt-3">
                    <button class="btn btn-info" onclick="updateWork(<?php echo $_GET['update_id'] ?>)">更新</button>
                    <button class="btn btn-secondary" onclick="host()">取消</button>
                </div>
            </div>
        <?php endif; ?>
        ...
        <?php if (isset($_GET['delete_id'])) : ?>
            ...
            <div class="alert alert-danger">
                <strong class="mr-3">删除</strong>确定要删除下面这条数据吗？
                <div class="btn-group">
                    <button class="btn btn-danger" onclick="deleteWork(<?php echo $_GET['delete_id'] ?>)">删除</button>
                    <button class="btn btn-secondary" onclick="host()">取消</button>
                </div>
            </div>

            <?php $workInfo = getWorkInfo($_GET['delete_id']);
        endif; ?>
    </div>
